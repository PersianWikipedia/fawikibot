...
def main(sqlnum):
    tasks = [
        ...
        },
        {
            "sqlnum":10,
            "sql":
            "SELECT /* SLOW OK */ CONCAT(':{{ns', CASE WHEN page_namespace = 0 THEN '' ELSE ':' END, page_namespace, '}}:', page_title), COUNT(*) FROM revision JOIN page ON page_id = rev_page GROUP BY page_namespace, page_title ORDER BY COUNT(*) DESC, page_title ASC LIMIT 1000;",
...
""",
            ...
            "pref": "این صفحه فهرستی از کاربران غیر رباتی است که در یک ماه اخیر ویرایش داشته‌اند؛ برای هر کاربر نسبت کل ویرایش‌هایش (از ابتدای ایجاد حساب) در فضاهای نام مختلف هم نشان داده شده‌است.\n[[رده:گزارش‌های دیتابیس ویکی‌پدیا]]" +
                    "\n\nآخرین به روز رسانی: ~~~~~",
