...
def checkdisambig(htmltxt):
    ...
    htmltxt = re.sub(ur'<div role="note" class="hatnote navigation-not-searchable">(.*?)</div>', u'',
                     htmltxt)  # حذف ابهام‌زدایی در الگو تغییرمسیر
...
def get_page_links(linktitle):
    ...
    try:
        links = requests.post('https://fa.wikipedia.org/w/api.php', params).json()['query']['pages'].values()[0][
            'links']
    except:
        time.sleep(2)
        try:
...
def clean_text(txt, remove_regex, faTitle):
    ...
    faTitle2 = re.sub(
        ur"([\(\)\^\%\$\#\@\&\,\=\{\[\}\]\'\|۱۲۳۴۵۶۷۸۹۰\?\.\!؟»«،\:؛\"\/\\\t\'\*\+\–\-\n0-9٬٫a-zA-Z\_\ـ])+", u' ',
        faTitle)
    faTitle_list = faTitle2.strip() + u' ' + faTitle.replace(u'‌', u'') + u' ' + faTitle.replace(u'‌',
                                                                                                 u' ') + u' ' + faTitle + u'‌ها' + u' ' + faTitle + u'‌های' + u' ' + faTitle + u'‌هایی' + u' ' + faTitle + u'ی' + u' '
...
def get_page(title):
    txt = u''
    try:
        txt = requests.post('https://fa.wikipedia.org/w/api.php',
                            params={"titles": title, "action": "query", "prop": "revisions",
                                    "rvprop": "content", "format": "json"}).json()['query']['pages'].values()[0][
            'revisions'][0]['*']
    except:
        time.sleep(2)
        try:
...
def check_grammer(faText_list, words_text_list):
    ...
    for item in faText_list:
        if item in words_text_list:
            continue
        item2 = re.sub(
            ur'(ب|ن|م|می|نمی|)‌?' + persianPastVerbs + u'(ه|)‌?(بوده?|شده?|می‌شود?|)(م|ی|یم|ید|ند|ام|ای|است|ایم|اید|اند|)',
            u'', item)
...
def main(faTitle, word):
    if (
            u'نظرخواهی' in faTitle or u'قهوه‌خانه' in faTitle or u'تابلو' in faTitle or u'/' in faTitle) and u'ویکی‌پدیا:' in faTitle:
    ...
    if faTitle:
        ...
        faText_list, clean_fa_text, Syntaxs_list, Syntaxs_list2, Erab_words, giumeh_place, txt_list2 = clean_text(
            faText, remove_regex, faTitle)
        faNewText = u' ' + u' '.join(faText_list) + u' '
        clean_fa_text2 = re.sub(ur'«[^»]+»', u' ', clean_fa_text)
        result = result + sorted(regex_maker(epithet_black_list, 0, clean_fa_text2, False, {}, faTitle),
                                 key=itemgetter('word'))
        ...
        result = result + sorted(regex_maker(Wrong_word_list, 2, clean_fa_text, True, {}, faTitle),
                                 key=itemgetter('word'))
        del clean_fa_text
    if word:
    ...
    Finall_rexult = {
        u"result": result + sorted(regex_maker(Fourth_step_words, 1, faNewText, False, hun_suggest, faTitle),
                                   key=itemgetter('word')),
        u"types": {u"0": {u"color": u'#9191ff', u"title": u'القاب ممنوع', u"autofix": True, u"syntax": False},
                   ...
                   u"4": {u"color": u'#68ff00', u"title": u'نویسهٔ غیراستاندارد (نیازمند ابرابزار)', u"autofix": True,
                          u"syntax": False},
...
def Open_json(OurResult, title):
    ...
    error_num = len(list0) + len(list1) + len(list2) + len(list3) + len(list4) + len(list5) + len(list6) + len(
        list7) - title_error_num
    if error_num > 1:
