...
def get_cat(link):
    link=link.replace(u' ',u'_')
    total_cache=dict(_cache_old,**_cache)
    if total_cache.get(tuple([link, 'get_cat'])):
        return total_cache[tuple([link, 'get_cat'])]
    params = {
        'action': 'query',
        'prop': 'categories',
        'titles': link,
    ...
    try:
        queryresult = query.GetData(params,faSite)  
        for item in queryresult[u'query'][u'pages']:
            cats=queryresult[u'query'][u'pages'][item][u'categories']
        _cache[tuple([link, 'get_cat'])]=cats
        return cats
    except: 
        _cache[tuple([link, 'get_cat'])]=[]
        return []
...
def addlable (workpage,resion,user,case):
    ...
    '''
    if case=='A':
        brif_resion=u'کوتاه + بدون میان‌ویکی'
    elif case=='B':
        brif_resion=u'کوتاه+با میان‌ویکی'
    elif case=='B2':
        brif_resion=u'کوتاه+بدون میان‌ویکی'
    elif case=='B3':
        brif_resion=u'کوتاه+بدون میان‌ویکی+ بدون منبع'
    elif case=='C':
        brif_resion=u'کپی‌کاری+بدون میان‌ویکی'
    elif case=='D':
        brif_resion=u'مقاله کوتاه در مورد انسان'
    else:
        brif_resion=u'دلیل نامشخص'
    wikipedia.output(u'->>>>>>>>>>>>>>>>>>>>>>>>'+user+u'--------------')
    msg=u'ربات-کاربر:هشدار [[ویکی‌پدیا:حذف زمان‌دار|حذف]] > [['+workpage+u']] > '+brif_resion
    try:
        page = wikipedia.Page(faSite,u'ویکی‌پدیا:گزارش دیتابیس/پیشنهاد برای حذف توسط ربات')
        text=page.get()
        if not u'[['+workpage+u']]' in text:
            text=text.replace(u'\r',u'')
            newRow=u'|[['+workpage+u']] || [[کاربر:'+user+u"]] ||{{جا:formatnum:"+str(Page_size(workpage))+u"}} ||'''"+ brif_resion+u"''' ||~~~~~\n|-\n"
            text=text.replace(u'{{/بالا}}',u'{{/بالا}}\n'+newRow)
            msg=u'پیشنهاد حذف برای [['+workpage+u']] > '+brif_resion
            page.put(text.strip(), msg)
    except:
        wikipedia.output(u'an error is accured!')
        pass
    try:
        ...
        if not u'[['+workpage+u']]' in text_user:
            pageuser.put(text_user+u'\n{{subst:کاربر:FawikiPatroller/هشدار احتمال حذف|'+workpage+u'|اهمیت='+resion+u'}} ~~~~', msg)
    except:
        wikipedia.output(u"couldn't open user page")
...
def senario_D(text,page_title):
    ...
    if rasio*main_page_size<300 and ClaimFinder(page_title,31)==u'انسان':
            return u'مقالهٔ کوتاه دربارهٔ انسان و نبود مطلب کافی برای مرتفع کردن شرایط [[وپ:سرشناسی]]','D'
    return False,'D'
    ...
    if not interwiki_list:
        if Page_size(page_title)<700:
            if check_admin(page_history):
                return u"مقالهٔ '''کوتاه و بدون میان‌ویکی''' است. برای نشان دادن سرشناسی و اهمیت موضوع به مطالب آن بیافزایید." ,'A'   
            else:
...
def senario_B(text,page_title,interwiki_list):
    ...
    if interwiki_list:    
        if Page_size(page_title)<500:
             return u"با وجودی که مقاله '''میان‌ویکی''' دارد و راحت می‌توانید آن را گسترش دهید '''ولی کوتاه است!''' لطفاً برای نشان دادن سرشناسی و درستی به مطالب مقاله بیافزایید.",'B'
    else:
        if Page_size(page_title)<200:
             return u'مقالهٔ خیلی کوتاه است برای مشخص شدن سرشناسی و اهمیت موضوع لطفاً مطالب بیشتری به مقاله بیافزایید.','B2'
        if Page_size(page_title)<300:
            if not (u'<ref' in text or u'{{یادکرد' in text or u'{{Citation' in text or u'{{citation' in text):
                return u"مقالهٔ '''کوتاه و بدون منبع''' است برای نشان دادن سرشناسی و اهمیت موضوع مقاله را گسترش دهید.",'B3'
    return False,'B'
...
def senario_C(text,page_title,interwiki_list):
    ...
    if not interwiki_list and text.find(u'[[')==-1 and text.find(u'منبع')==-1 and text.find(u'منابع')==-1 and text.find(u'<ref')==-1 and text.find(u'{{یادکرد')==-1 and text.find(u'{{Citation')==-1 and text.find(u'{{citation')==-1:    
        return u"احتمال [[وپ:کپی‌کاری|کپی‌کاری]] وجود دارد متن مقاله را بازنویسی کنید و '''میان‌ویکی و رده''' مورد نیاز را هم بیافزایید.",'C'
    return False,'C'
...
def senario_E(text,user,workpage_page,interwiki_list):
    ...
    if textcat.find(u'[[رده:')==-1 and textcat.find(u'[[category:')==-1 and textcat.find(u'[[Category:')==-1 and not get_cat(fapage_title):
        alarmnum+=1
        force=True
        alarm_message=At_faArticle
        ...
        if len(interwiki_list)>0 and enpage_title:#اگر میان‌ویکی داشت
            ...
            global category_needed
            category_needed+=encats
        wikipedia.output(alarm_message)
        alarmtup.append(alarm_message)
        alarm_text+=alarm_message+u'|'
    ...
    if toppage.strip() and workpage_page.find(u'فهرست')==-1 :#10
        ...
        if (text.find(u'.wikipedia.org')!=-1 or text.find(u'*ویکی')!=-1 or text.find(u'* ویکی')!=-1):
            if len(interwiki_list)<1:#11
                ...
                alarmtup.append(alarm_message)
                alarm_text+=alarm_message+u'|'
                global interwiki_neede
                interwiki_neede.append(fapage_title)
    if not check_user_editnum(user,2000):
        for i in enchar+u',0123456789'+u"'":#12
            if workpage_page.find(i)!=-1 or workpage_page.find(i.lower())!=-1:
                alarmnum+=1
                alarm_message=At_faArticle
                alarm_message+=u"عنوان مقاله را با نویسه و اعداد فارسی ترجمه یا آوانگاری نمایید؛ در ویکی‌پدیای فارسی [[وپ:عنوان|عنوان مقاله‌ها]] باید با نویسه و اعداد فارسی نوشته شوند و فقط برای موارد خاصی استثنا وجود دارد."
                force=True
                wikipedia.output(alarm_message)    
                alarmtup.append(alarm_message)
                alarm_text+=alarm_message+u'|'
...
def pagecondition(text,page_history,user,page_title):
    ...
    workpageok,case=senario_D(text,page_title)
    if workpageok:
        return workpageok,case
    workpageok,case=senario_A(text,page_history,page_title,interwiki_list)
    if workpageok:
        return workpageok,case
    workpageok,case=senario_B(text,page_title,interwiki_list)    
    if workpageok:
        return workpageok,case
    workpageok,case=senario_C(text,page_title,interwiki_list)
    if workpageok:
        return workpageok,case
...
def add_lists_to_wiki():
    login_fa(u'FawikiPatroller')
    if interwiki_neede:
        try:
            statpage = wikipedia.Page(faSite,u'کاربر:FawikiPatroller/فهرست مقالات نیازمند میان‌ویکی')
            text_stat=statpage.get().strip()
            text_stat_list=u'\n# [['.join(interwiki_neede)
            text_stat_list=text_stat_list.replace(u'\n# [[',u']]\n# [[')+u']]'
            text_stat_list=text_stat_list.strip()
            if not text_stat_list in text_stat:
                text_stat=text_stat+u'\n# [['+text_stat_list
                statpage.put(text_stat,u'ربات:به‌روزرسانی فهرست مقالاتی که ممکن است میان‌ویکی داشته باشند')
            
        except:
            pass
    if category_needed:
        try:
            statpage = wikipedia.Page(faSite,u'کاربر:FawikiPatroller/فهرست رده‌های همسنگی که نیاز داریم')
            text_stat=statpage.get().strip()
            text_stat_list=u'\n# '.join(category_needed)
            text_stat_list=text_stat_list.replace(u"'''",u'').replace(u"'''",u'')
            text_stat_list=text_stat_list.strip()
            if not text_stat_list in text_stat:
                text_stat=text_stat+u'\n'+text_stat_list
                statpage.put(text_stat,u'ربات:به‌روزرسانی فهرست رده‌های همسنگی که نیاز داریم')
        ...
        if timestamp<todaynum-2:
            ...
            if check_user(user):# if the first user is not bot and article is from 2 days ago it will continue
                ...
                if counter>1000:#number of check for each time
                   break
                text33=text.replace(u'_',u' ')
                if  text33.find(u'نمایه}}')!=-1 or text33.find(u'{{نمایه')!=-1 or text33.find(u'{{نام معمول')!=-1 or text33.find(u'{{فهرست شیمی')!=-1 or text33.find(u'{{نام کوچک')!=-1 or text33.find(u'{{نام خانوادگی')!=-1 or text33.find(u'ابهام‌زدائی')!=-1 or text33.find(u'ابهام زدائی')!=-1 or text33.find(u'حذف سریع')!=-1 or text33.find(u'{{حس')!=-1 or text33.find(u'{{حس-')!=-1 or text33.find(u'{{انتقال به')!=-1 or text33.find(u'ادغام تاریخچه')!=-1 or text33.find(u'ادغامت')!=-1 or text33.find(u'disambig')!=-1 or text33.find(u'Disambig')!=-1 or text33.find(u'ابهام زدایی')!=-1 or text33.find(u'ابهام‌زدایی')!=-1 or text33.find(u'حذف زمان‌دار/پیغام')!=-1 or text33.find(u'در دست ویرایش ۲')!=-1 or text33.find(u'{{پیشنهاد حذف۲')!=-1:
                    continue
                resion,case=pagecondition(text,page_history,user,workpage)
                if resion:
                    login_fa(u'FawikiPatroller')
                    addlable(workpage,resion,user,case)
                    login_fa(u'Rezabot')    
            else:
                wikipedia.output(u'User was bot')
                continue
